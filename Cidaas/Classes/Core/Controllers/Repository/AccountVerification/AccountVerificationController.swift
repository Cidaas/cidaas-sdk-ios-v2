...
public class AccountVerificationController {
    ...
    public func getAccountVerificationList(sub: String, properties: Dictionary<String, String>, callback: @escaping(Result<AccountVerificationListResponseEntity>) -> Void) {
        ...
        // call verifyAccount service
        AccountVerificationService.shared.getAccountVerificationList(sub: sub, properties: properties) {
            switch $0 {
            case .failure(let error):
                // log error
                let loggerMessage = "Verify account list service failure : " + "Error Code - " + String(describing: error.errorCode) + ", Error Message - " + error.errorMessage + ", Status Code - " + String(describing: error.statusCode)
                logw(loggerMessage, cname: "cidaas-sdk-error-log")
            ...
            case .success(let serviceResponse):
                // log success
                let loggerMessage = "Verify account list service success : " + "Success - " + String(describing: serviceResponse.success)
                logw(loggerMessage, cname: "cidaas-sdk-success-log")
