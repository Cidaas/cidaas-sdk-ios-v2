...
public class UserActivityService {
    ...
    public func getUserLoginInformation(accessToken : String, userInfo: UserLoginInfoEntity, properties : Dictionary<String, String>, callback: @escaping (Result<UserLoginInfoResponseEntity>) -> Void) {
            ...
            case .failure(let error):
                ...
                if (response.data != nil) {
                    ...
                    do {
                        ...
                        // return failure
                        callback(Result.failure(error: WebAuthError.shared.serviceFailureException(errorCode: WebAuthErrorCode.USER_LOGIN_INFO_SERVICE_FAILURE.rawValue, errorMessage: errorResponseEntity.error.error, statusCode: Int(errorResponseEntity.status), error: errorResponseEntity)))
                    }
                    catch(let error) {
                        // return failure
                        callback(Result.failure(error: WebAuthError.shared.serviceFailureException(errorCode: WebAuthErrorCode.USER_LOGIN_INFO_SERVICE_FAILURE.rawValue, errorMessage: error.localizedDescription, statusCode: 400)))
                    }
                }
                else {
                    // return failure
                    callback(Result.failure(error: WebAuthError.shared.serviceFailureException(errorCode: WebAuthErrorCode.USER_LOGIN_INFO_SERVICE_FAILURE.rawValue, errorMessage: StringsHelper.shared.USER_LOGIN_INFO_SERVICE_FAILURE, statusCode: response.response?.statusCode ?? 400)))
                }
                return
            case .success:
                if response.response?.statusCode == 200 {
                    if let jsonString = response.result.value {
                        ...
                        do {
                            let data = jsonString.data(using: .utf8)!
                            let userInfoEntity = try decoder.decode(UserLoginInfoResponseEntity.self, from: data)
                            // return success
                            callback(Result.success(result: userInfoEntity))
                        }
                        catch {
