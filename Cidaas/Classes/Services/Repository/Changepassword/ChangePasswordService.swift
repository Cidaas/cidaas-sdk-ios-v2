            ...
            case .failure:
                if (response.data != nil) {
                    ...
                    do {
                        ...
                        // return failure
                        callback(Result.failure(error: WebAuthError.shared.serviceFailureException(errorCode: WebAuthErrorCode.CHANGE_PASSWORD_SERVICE_FAILURE.rawValue, errorMessage: errorResponseEntity.error.error, statusCode: Int(errorResponseEntity.status), error: response.value ?? response.error?.localizedDescription ?? "")))
                    }
                    catch(let error) {
                        // return failure
